...
export class LandService {
    ...
    async createLand(areaId: string, dto: LandCreateDto, files: Express.Multer.File[]): Promise<any> {
        ...
        const imageName = await this.storageService.uploadMultiFiles(ImagePath.CARD_LAND, files);
    ...
    async getLandById(id: string): Promise<Land> {
        const land = await this.landRepository.createQueryBuilder('land')
            .where('land.id = :id', {id})
            .leftJoinAndSelect('land.soilType', 'soilType')
            .leftJoinAndSelect('land.productType', 'productType')
            .getOne();
    ...
    async uploadImage(landId: string, dto: UploadLandDto, images: Express.Multer.File[]): Promise<Land> {
        ...
        if (!isImages) {
            ...
            const deleteImages = MulterUtils.convertPathToUrl(images.map(image => image.path));
            await Promise.all(deleteImages.map(image => MulterUtils.deleteFile(image)));
            throw new ApiException(ErrorMessages.IMAGE_NOT_FOUND);
        }
        // xóa ảnh cũ đi
        await Promise.all(dto.fileNames.map(fileName => MulterUtils.deleteFile(fileName)));
        land.images = land.images
            // xóa ảnh cũ đi
            .filter(image => !dto.fileNames.includes(image))
            // thêm ảnh mới vào
            // path cover url
            .concat(MulterUtils.convertPathToUrl(images.map(image => image.path)));
